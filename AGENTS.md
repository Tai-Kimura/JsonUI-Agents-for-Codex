...
5. **Conditional logic and hand-written code are declared and tested.** `branchContracts` / `unitContracts` where the entry makes a real test exist, then `jsonui-test generate branch-tests --check` and `unit-stubs --check` exit 0. A method with no branches and nothing to assert gets no entry.
