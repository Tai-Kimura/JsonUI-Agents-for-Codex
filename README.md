...
A collection of specialized agents and skills for OpenAI Codex CLI to support JsonUI framework development across iOS (SwiftUI/UIKit), Android (Compose/XML), and Web (React/Next.js).

> This is a port of [JsonUI-Agents-for-claude](https://github.com/Tai-Kimura/JsonUI-Agents-for-claude) adapted for OpenAI Codex CLI.

## Prerequisites

- [OpenAI Codex CLI](https://github.com/openai/codex) installed
- Multi-agent feature enabled (see Installation)
...
Multi-agent support must be enabled in Codex CLI:

**Option 1: Via CLI**
```
/experimental
# Then toggle "Multi-agents" ON
```
...
After requirements are complete, **start a new Codex CLI session** and run `Read AGENTS.md` again, then select Option 2.

#### Option 2: Implementation

When requirements are already defined, this switches to the `jsonui-orchestrator` agent which guides you through the implementation flow.

## Directory Structure

```
.codex/
  config.toml             # Agent role definitions
  agents/                 # Agent configuration files (.toml)
    jsonui-orchestrator.toml
    jsonui-requirements.toml
    jsonui-spec.toml
    jsonui-setup.toml
    jsonui-screen-impl.toml
    jsonui-test.toml
.agents/
  skills/                 # Implementation skills
    jsonui-layout/SKILL.md
    jsonui-viewmodel/SKILL.md
    ...
...
Agents orchestrate workflows and coordinate skills. Defined as roles in `.codex/config.toml`.
...
Use the `/agent` slash command to switch between agent threads, or let the orchestrator guide you.

## Skills

Skills execute specific tasks. Agents invoke skills as needed using the `$skill-name` syntax.
...
jsonui-requirements agent
  ...
  Start new Codex CLI session

[Session 2: Implementation]
Read AGENTS.md -> jsonui-orchestrator agent
  Step 1: jsonui-spec agent (review/refine specs)
  Step 2: jsonui-setup agent (project configuration)
  Step 3: jsonui-screen-impl agent (implementation)
  Step 4: jsonui-test agent (testing)
```
...
| Agent definitions | `.claude/agents/*.md` | `.codex/config.toml` + `.codex/agents/*.toml` |
| Skills directory | `.claude/skills/` | `.agents/skills/` |
| Rules | `.claude/rules/*.md` | Merged into `AGENTS.md` |
| Skill invocation | `/skill-name` | `$skill-name` |
| Agent switching | User manually launches agents | `/agent` command or automatic orchestration |
| Multi-agent | Built-in Task tool | Requires `multi_agent = true` feature flag |
...
Quality = Spec Quality x Prompt Skill x Context Management
...
```
JsonUI Approach:
Quality = Spec Quality x Code Constraints x Specialized Agents
...
- [JsonUI-Agents-for-claude](https://github.com/Tai-Kimura/JsonUI-Agents-for-claude) - Original Claude Code version
