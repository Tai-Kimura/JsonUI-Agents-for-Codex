...
source_commit: 10f8bee4f6136ef942f703baa80c1a566f4c9256

- **Last sync date:** 2026-09-25
- **Source commit subject:** `agents (jsonui-test B2.4): on Android, read the branch-test console lines from the test results XML — a runtime generated by jsonui-cli 1.8.119 or earlier prints them where Gradle's console does not show a passing test's output`
- **Note on this sync:** ports one commit: 10f8bee (jsonui-test B2.4: on Android, a runtime generated by jsonui-cli 1.8.119 or earlier prints the branch-test console lines where Gradle's console does not show a passing test's output — read them in the test results XML). `agents/test.toml` regenerated from the Claude source; the sentence has no agent reference, so the text is identical. The syncs before it: e063601 (the vitest reporter paragraph) and fee2646 (the eight units landed with jsonui-cli 1.8.119).
